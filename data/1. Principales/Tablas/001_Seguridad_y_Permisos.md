...
A continuación se modela el flujo lógico en el tiempo (_Sequence Diagram_) para una operación transaccional común (ej. Guardar una Factura).
...
- _Nota:_ El proceso detallado de cómo, cuándo y de dónde se extraen los datos para construir este JSON en memoria pertenece de forma exclusiva a la arquitectura del **Proceso de Inicialización de Sesión**, el cual se ejecuta una única vez durante el Login.
...
Aquí se evidencia el comportamiento _polimórfico_ del motor: la evaluación cambia lógicamente dependiendo de si lo que se está validando es un Menú (`004_OPC_MEN`) o una Acción Transaccional sobre datos (`004_CAT_TAB`).
